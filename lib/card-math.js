...
export function calculateSpecular(pose, face = 'front') {
  ...
  const sidewaysTilt = Math.abs(normal[0]);
  const angleDrivenClearcoat = sidewaysTilt * 0.6 * Math.pow(1 - sidewaysTilt, 5);
  ...
  return {
    ...
    strength: round(clamp(
      0.025 + angleDrivenClearcoat + Math.max(0, broadWeight - 0.006) * 6 + normalizedWeight * 18,
      0.025,
      0.34
    ))
  };
